  ...
  submitBtn.addEventListener("click", () => {
    ...
    if (userInput) {
      ...
      if (userInput == answerValue) {
        stopGame(`Yeah!! <span>benar sekali</span>, selamat kamu telah menjawab dengan benar..`);
      }
      //If user inputs operator other than +,-,*
      else if (operatorQuestion && !operators.includes(userInput)) {
        errorMessage.classList.remove("hide");
        errorMessage.innerHTML = "Mohon masukan operator yang benar...";
      }
      //If user guessed wrong answer
      else {
        stopGame(`upss Jawabanmu!! <span>  salah</span> coba lagi..`);
      }
    }
    //If user input is empty
    else {
      errorMessage.classList.remove("hide");
      errorMessage.innerHTML = "Terjadi kesalahan, Isi terlebih dahulu agar bisa melanjutkannya ....";
    }
  });
};
